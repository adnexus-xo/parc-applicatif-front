...
function RapportsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Export PDF - Support et Admin uniquement */}
      {!isLecteur && (
        <div className="grid grid-cols-2 gap-5">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 flex flex-col gap-4">
            ...
            <p className="text-sm text-gray-600">Exportez un tableau PDF contenant toutes les applications avec leur nom, version, état, criticité, catégorie et équipe.</p>
            <div className="flex items-center justify-between mt-2">
              <div></div>
              <button onClick={exportInventairePDF} disabled={loading} className="flex items-center gap-2 px-4 py-2.5 bg-[#0D1B6E] text-white rounded-xl text-sm font-medium hover:bg-[#E6BC00] transition-colors">
                <Download size={15} />
                Télécharger PDF
              </button>
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 flex flex-col gap-4">
            ...
            <p className="text-sm text-gray-600">Exportez un rapport PDF avec les chiffres clés du parc applicatif : totaux, répartition par état et criticité.</p>
            <div className="flex items-center justify-between mt-2">
              <div></div>
              <button onClick={exportSynthesePDF} disabled={loading} className="flex items-center gap-2 px-4 py-2.5 bg-[#FFD100] text-[#0D1B6E] rounded-xl text-sm font-medium hover:opacity-90 transition-colors">
                <Download size={15} />
                Télécharger PDF
              </button>
            </div>
      ...
      {/* Aperçu inventaire - visible par tous */}
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div className="px-5 py-4 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-800">Aperçu de l'inventaire</h2>
        </div>
