...
function ApplicationEditPage() {
  ...
  useEffect(() => {
    ...
    api.get(`/applications/${id}`).then((res) => {
      ...
      setForm({
        ...
        dateCreationFiche: app.dateCreationFiche?.split('T')[0] || '',
        dateMiseEnService: app.dateMiseEnService || '',
        dateObsolescence: app.dateObsolescence || '',
        categorie: { idCategorie: app.categorie?.idCategorie || '' },
        equipeUtilisatrice: { idEquipe: app.equipeUtilisatrice?.idEquipe || '' },
        equipeResponsable: { idEquipe: app.equipeResponsable?.idEquipe || '' },
      });
    });
  }, [id]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="flex justify-center mt-4  text-2xl font-bold text-gray-900">Modifier l'application</h1>
        <h2 className="text-[#0D1B6E] text-sm mt-1 font-medium">Modifiez les informations de la fiche application</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-6">
        ...
        <div>
          ...
          <div className=" grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="nomApplication" value={form.nomApplication} onChange={handleChange} required className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Version *</label>
              <input name="version" value={form.version} onChange={handleChange} required className={inputClass} />
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                style={{color: !form.categorie.idCategorie ? '#9CA3AF' : '#374151'}}
            ...
            <div>
              ...
              <select
                ...
                required
                className={inputClass}
                style={{color: !form.criticite ? '#9CA3AF' : '#374151'}}
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <select
                name="idCategorie"
                value={form.categorie.idCategorie}
                onChange={handleChange}
                required
                className={inputClass}
                style={{color: !form.categorie.idCategorie ? '#9CA3AF' : '#374151'}}
            ...
            <div>
              ...
              <select
                name="idEquipeUtilisatrice"
                value={form.equipeUtilisatrice.idEquipe}
                onChange={handleChange}
                required
                className={inputClass}
                style={{color: !form.equipeUtilisatrice.idEquipe ? '#9CA3AF' : '#374151'}}
              >
            ...
            <div>
              ...
              <select
                name="idEquipeResponsable"
                value={form.equipeResponsable.idEquipe}
                onChange={handleChange}
                required
                className={inputClass}
                style={{color: !form.equipeResponsable.idEquipe ? '#9CA3AF' : '#374151'}}                
              >
                <option value="">Sélectionner une équipe</option>
                {equipes.map((eq) => (
                  <option key={eq.idEquipe} value={eq.idEquipe}>{eq.nomEquipe}</option>
        ...
        <div className="flex justify-end gap-3 pt-2">
          <button type="button" onClick={() => navigate(`/applications/${id}`)} className="px-5 py-2.5 rounded-xl border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
            Annuler
          </button>
